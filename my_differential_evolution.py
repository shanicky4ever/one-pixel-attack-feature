...
__all__ = ['my_differential_evolution']

_MACHEPS = np.finfo(np.float64).eps

def my_differential_evolution(func, bounds, operation,args=(), strategy='best1bin',
    ...
    solver = DifferentialEvolutionSolver(func, bounds, operation,args=args,
...
class DifferentialEvolutionSolver(object):
    ...
    def __init__(self, func, bounds, operation ,args=(),
        ...
        self.num_population_members = max(5, len(self.operation) * self.parameter_count)
    ...
    def init_population_feature(self):

        """
        Initialises the population at feature
        """
        #rng = self.random_number_generator
        #self.population = r    ng.random_sample(self.population_shape)
        self.population = self.operation
    ...
    def __next__(self):
        ...
        itersize = max(0, min(len(self.operation), self.maxfun - self._nfev + 1))
    ...
    def _select_samples(self, candidate, number_samples):
        ...
        """
        idxs = list(range(int(self.num_population_members/self.parameter_count)))
